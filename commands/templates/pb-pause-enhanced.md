...
### Step 6.5: Verify Active BEACONs (NEW - v2.12.0)
...
- [ ] **All 9 BEACONs verified and documented** ← NEW
...
/pb-start → /pb-cycle → /pb-commit → /pb-ship
     ...
     └─────────│   BOUNDARY  │       Reviews →
               └─────────────┘       PR → Merge →
                     ↑               Release
                     ↓
              /pb-resume ←──────── /pb-pause ← v2.12.0 NOW WITH BEACON VERIFICATION
              (recover)            (preserve)
```
...
- **`/pb-pause`** → Gracefully pause work (with BEACON verification) ← YOU ARE HERE
...
- `/pb-resume` — Get back into context after a break (now includes BEACON loading)
...
- **Verify all 9 BEACONs are active** ← NEW
...
2. Sync with remote
3. **Load all 4 context layers in order** ← NEW
4. **Confirm all 9 BEACONs active** ← NEW
5. Review working context
6. Read handoff notes
7. Verify environment
8. Run tests
9. Continue from documented next steps

...
- **`/pb-review-incoming` v1.2.0: a third mode, for the consumer side of a release.** Four directions exist between a project and a sibling library; the command owned three and the fourth -- *a consumer adapting to a release that was just cut* -- was covered nowhere. That work is not a version bump: read the changelog, diff what you actually consume, decide, migrate config, re-pin, update your contract checks, re-run gates. **Release mode** resolves scope to a version delta paired with the consumed surface, inherits the empty-resolution hard stop (*"already on that version"* is an answer, not a clean intake), and states plainly that **what you consume and what was published may differ** -- a SHA pin, a fork, a vendored patch, a pre-release, a registry artifact that disagrees with its own tag. Step 5 gains **adopt / pin / skip** per unit of the delta, with the per-commit table now explicitly scoped to range mode, and a **required digest**: concerns surviving adoption is mandatory with no empty case, because it exists in no other artifact and evaporates the moment a green intake feels finished; highlights and adjustments are required when non-empty, since forcing a line into an empty section teaches filler. Step 3 gains a mandatory release-notes-versus-code check built on the mechanism already used for the register -- extract, compare, cite -- because a release note can be wrong in a way your tests cannot catch, your tests having encoded what you believed the note said. Two things the handoff that requested this did not have. **The security signal list already contained *new or bumped third-party dependencies*,** which is the entire subject matter of a release intake, so read literally it fires on every run -- a rule that always fires carries no information and discredits its neighbours, so release mode narrows it to transitive-dependency changes, new egress, new subprocess or build hooks, changed parsing of untrusted input, and maintainer or signing changes. And **the founding asymmetry does not transfer**: a published artifact carries its own contract and was authored by people who owe you nothing about your conventions, so *trust the content* -- which is derived from that asymmetry -- is not a licence in release mode, and the command now says so where a reader will hit it. Every forking step answers all three modes; implied was previously guessed. `/pb-learn` v1.1.0 lands in the same commit rather than "separately", since separately is how a cross-reference becomes never: a session looking for dependency intake reached for it and guessed wrong, so its When-NOT-to-Use now names the right door (`pb-cycle` swapped out of Related Commands to hold the ceiling of 5; it carried no back-link). **The claim is scoped:** the evidence is demand from one project that did the work by hand, not evidence the shape works. That gets tested the first time someone runs it.

...
- **`/pb-storage` v1.1.0 and `/pb-doctor` v1.1.0: the disk check measured a volume that never moves, and the "safe" tier could destroy a model file.** Three defects, all found by running the commands for real on a macOS dev machine rather than reading them. **(1)** Both commands ran `df -h /`, but on APFS `/` is the read-only signed *system* volume and its capacity percentage is effectively constant — measured live, `df -h /` reported **24%** while `/System/Volumes/Data`, which holds everything, was at **84%**. The documented "run when disk exceeds 80%" trigger could not fire. Both now measure the Data volume, with a note that `Avail` is shared between the two while `Used` and `Capacity` are not. **(2)** Tier 1 listed `~/.cache/*` as *"Always reversible, no side effects"* and shipped a blanket `rm -rf`. `~/.cache` is a convention, not a guarantee: it holds downloaded model weights, provisioned VM images and vendored toolchains. On the machine in question it held `whisper-cpp/ggml-base.en.bin`, a hand-downloaded speech model a project depended on for verification — deleting it would have broken that pipeline silently, and "the cache regenerates" is exactly why nobody would have looked there. The tier now requires enumerating `~/.cache` and excluding by name, and the glob is gone from every block including the nuclear one. **(3)** `/pb-doctor` used GNU `ps aux --sort=` in five places, which macOS `ps` rejects outright — on a playbook whose own header says *"Platform: macOS"*. Replaced with BSD forms (`ps -Ao pcpu,pmem,comm -r`). Deletion commands also moved off globs onto `find -delete`, because in zsh a glob matching nothing (an empty `~/.Trash`) aborts the entire command before any of it runs — so a tidy one-liner deletes nothing and reports success.

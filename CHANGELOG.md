...
- **`/pb-preflight` v1.1.0: `N/A` is no longer a writable outcome.** A bullet outside your surface is not rendered at all; the rest resolve to PASS, FAIL or DELEGATED (provider, verified date). Bullets are renumbered `N.M`.
- **Regenerated CLAUDE.md files lose sections.** Global drops Development Ritual, Session Ritual and the Model Selection paragraph (`/pb-claude-global` v2.9.0). Project drops Testing, Environment, Session Quick Start and all per-commit state (`/pb-claude-project` v1.4.0). Hand-written content under `## Custom (Manual)` is carried across.

### Added

- **`/pb-evolve` v1.4.0: help-or-constrain, every cycle (Step 3.5).** Does each surface still structure and check the model's work, or hold a stronger model back? Facts come from fetched vendor docs, not recall. The review is two-sided: name where the model needs more structure before proposing a cut. A rule is deleted on documentation or observed sessions, never on the model's account of its own defaults. It edits what commands say; retiring a command stays a human decision.
- **`/pb-preflight` v1.1.0: surface assessment.** Step 0 picks a profile (Service or Static/Managed) or declares and justifies another, so the gate shows only the bullets that apply. DELEGATED names who owns a concern you do not operate, and when you last checked.
- **The metadata pipeline runs in CI**, with two invariants: extraction covers every command, and the validator rejects a malformed record. It runs non-`--strict` because the extractor still reads fenced sample blocks as command references.
- **`TestTemplateCurrency`**: no model version strings in either CLAUDE.md generator template. Tests 115 → 121.

### Changed

- **`/pb-claude-global` v2.9.0.** Verification stays as done-criteria ("done is shown, not asserted") and goes as instruction: current Opus verifies its own work, and telling it to verify again costs tokens without adding quality. "Confirm multi-file changes before acting" becomes a scope rule. The paragraph naming a model version is deleted, not refreshed. The generator gains Step 0, so Custom (Manual) blocks survive regeneration, which the template had promised without a mechanism.
- **`/pb-claude-project` v1.4.0.** The generated file records only what a reader would not find in the repo in one pass: commands that work, generated paths, guardrails, conventions. Per-commit state is banned and the working context is linked instead. The template no longer asks for the status line its own rules forbid. `/pb-greenfield` v2.0.1 follows.
- **One CLAUDE.md size figure: under 200 lines**, Anthropic's published target, for both files. Token budgets, summed totals and BEACON-heading counts are gone; the prefix is a reader's marker with no mechanical effect. `/pb-context-review` v2.3.0, `/pb-pause` v1.9.3, `/pb-resume` v1.9.2.
- **`/pb-claude-orchestration` v1.5.0.** Delegation is capped: large, genuinely independent tracks only, and never a subagent to check your own work. Harness facts are dated instead of named after a model.
- **`/pb-forge` v1.3.0.** Self-gate sub-stage 2 is offered, not run. The assistant can now start `/code-review` itself, and an automatic extra self-check on every diff over the trigger is over-verification. Forge and `/pb-ship` v1.2.1 now link `/pb-preflight` both ways.
- **`/pb-huddle` v1.5.0.** Briefs, sketches and summaries are claims about the work: open what they rest on before briefing the panel.
- **`/pb-pause` v1.9.3.** The recap stays in the pause notes and `/pb-resume` archives it. Archiving at pause made resume skip the review the archive exists for.
- **FAQ** states the breaking-change policy the project follows: recorded per command in `breaking_changes`, listed under Breaking, in any release.

### Removed

- **The voice-block convention**, at 3 of 118 commands after seven months. `/pb-start` v2.3.1 drops its block; `/pb-review` v2.7.1 keeps the output decision as a plain line.
- **Four superseded metadata scripts** (720 lines) and two import symlinks. One script-loading mechanism remains.

### Fixed

- **`install.sh` left a dead link behind whenever a command was renamed or retired.** It now prunes links into the repo whose source is gone.
- **The metadata validator rejected all 7 `commands/utilities/` commands.** `VALID_CATEGORIES` is the single source, pinned to the directory listing by a test.
- **`book/llms.txt` listed 114 commands.**
